...
public class Omok extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("omok doPost 접속!");
		// 1. 내가 돌을 가져왓을때
		if (action.equals("/sendMyInfo.do")) {
	        System.out.println("=====================sendMyInfo=====================");
			myStoneSet(request, response);
	        System.out.println("=====================sendMyInfo=====================");
		} // 2. 상대가 돌을 놓았을때
		else if (action.equals("/sendYouInfo.do")) {
	        System.out.println("=====================sendYouInfo=====================");
			oppStoneSet(request, response);
	        System.out.println("=====================sendYouInfo=====================");

		} // 3. 정보 검색
		else if (action.equals("/requUserInfo.do")) {
	        System.out.println("=====================requUserInfo=====================");
			winRateCalc(request,response);
	        System.out.println("=====================requUserInfo=====================");
		} // 4. 시스템 메시지 수신
		else if (action.equals("/sendMemStatus.do")) {
	        System.out.println("=====================sendMemStatus=====================");
	...
	private void winRateCalc(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int oppoNo  = Integer.parseInt(request.getParameter("oppno"));
		if(rankDao == null)
			rankDao = new RankDAO();
		Map<Integer, Object> playerMap;
		playerMap = rankDao.winRate(playerNo);
		Map<Integer, Object> oppoMap;
		oppoMap = rankDao.winRate(oppoNo);
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("NAME", String.valueOf(playerMap.get(0)));
		jsonObject.put("winCnt", (int)playerMap.get(1));
		jsonObject.put("loseCnt", (int)playerMap.get(2));
		jsonObject.put("winRate", (double) playerMap.get(3));
		
		JSONObject jsonObject2 = new JSONObject();
		jsonObject2.put("NAME", String.valueOf(oppoMap.get(0)));
		jsonObject2.put("winCnt", (int)oppoMap.get(1));
		jsonObject2.put("loseCnt", (int)oppoMap.get(2));
		jsonObject2.put("winRate",(double) oppoMap.get(3));
	...
	private void VictoryLogSetting(HttpServletRequest request, HttpServletResponse response, int wintype) {
		...
		int winPlayerNo = Integer.parseInt(request.getParameter("player"));
		int losePlayerNo = Integer.parseInt(request.getParameter("opponent"));
		String LogDetail;
		if(wintype == 1) {
	...
	private void myStoneSet(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("my stone: x: "+x+" y: "+y);

		int stonecolor = Integer.parseInt(request.getParameter("stone"));
		...
		boolean success = board.myStoneplay(x, y,stone);
		...
		// 돌을 놓은 후의 상태 체크
		if (success) {
			Omok = board.checkOmok(stone);
			Six = board.checkSix(x, y,stone);
			DoubleThree = board.checkDoubleThree(x, y,stone);
			System.out.println("결과:"+success +"[omok]:  "+Omok +" [Six]: "+Six+" [쌍삼]: "+DoubleThree);
			board.Print();
			if (Omok) {
				message = "W";
			} else if (Six) {
	...
	private void oppStoneSet(HttpServletRequest request, HttpServletResponse response)
		...
		int y = Integer.parseInt(request.getParameter("y_col"));
		System.out.println("opp stone: x: "+x+" y: "+y);
