...
public class Omok extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	   ...
	   String WinCount = request.getParameter("Win");
	   if (WinCount != null) {
		   win_count++;
	   }
	   
	   String LoseCount = request.getParameter("Lose");
	   if (LoseCount != null) {
		   lose_count++;
	   }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 Board board = (Board) context.getAttribute("board");  
	        ...
	        String stone = WhoIsPlayer.equals("black_player") ? "B" : "W"; 
		    ...
		    // 돌을 놓은 후의 상태 체크
		    if (success) {
		    	Omok = Board.checkOmok(x, y, WhoIsPlayer);
		    	Six = Board.checkSix(x, y, WhoIsPlayer);
		    	DoubleThree = Board.checkDoubleThree(x, y, WhoIsPlayer);
		        
		        if (Omok) {
		            message = WhoIsPlayer + " 승리!";
		        } else if (Six) {
		            message = "6X";
